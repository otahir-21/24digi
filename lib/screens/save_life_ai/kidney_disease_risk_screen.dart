...
class KidneyDiseaseRiskScreen extends StatefulWidget {
  const KidneyDiseaseRiskScreen({super.key});

  @override
  State<KidneyDiseaseRiskScreen> createState() =>
      _KidneyDiseaseRiskScreenState();
}

class _KidneyDiseaseRiskScreenState
    extends State<KidneyDiseaseRiskScreen> {
      ...
      child: Row(
        children: [
          Container(
            ...
            child: const Icon(
              Icons.thumb_up_alt_outlined, // Badge from image
              color: _C.green,
              size: 20,
            ),
          ),
          ...
          const Column(
            ...
            children: [
              Text(
                'Kidney Disease Risk Indicators',
                style: TextStyle(
                  color: _C.white,
                  fontSize: 17,
          ...
          children: [
            ...
            Row(
              children: const [
                ...
                SizedBox(width: 6),
                Text(
                  'All indirect indicators within range',
                  style: TextStyle(
                    color: _C.green,
                    fontSize: 12,
                    fontWeight: FontWeight.w500,
...
class _TrendChart extends StatelessWidget {
  ...
  static const _actual = [75.0, 76.0, 74.0, 75.5, 76.2, 74.8, 75.0];
  static const _pred = [75.2, 75.8, 74.5, 75.2, 76.0, 75.0, 75.2];
  static const _base = [75.0, 75.0, 75.0, 75.0, 75.0, 75.0, 75.0];
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            const Text(
              'Circulatory & Hydration Inference',
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _section('What was detected', 'SafeLife monitors indirect kidney health indicators through circulatory patterns, hydration estimation, and blood pressure trends. All monitored parameters are currently within normal ranges.'),
            _section('Why it matters', 'Chronic kidney disease often develops silently. While wearable data cannot directly measure kidney function, certain circulatory and metabolic patterns can suggest increased risk.'),
            _section('Short-term meaning', 'Your circulatory patterns and estimated hydration levels suggest healthy kidney function indicators.'),
            _section('Long-term prediction', 'No kidney-related risk signals detected. Regular clinical testing (blood and urine tests) remains the gold standard for kidney health assessment.'),
            const SizedBox(height: 16),
            GestureDetector(
              onTap: onToggle,
              child: Container(
...
class _ContributingFactors extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final factors = [
      {'label': 'Blood Pressure', 'low': true},
      {'label': 'Hydration', 'low': true},
      {'label': 'Cardiovascular Recovery', 'low': true},
    ];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
...
class _RecommendedActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final actions = [
      {'tag': 'Lifestyle', 'text': 'Maintain adequate daily hydration (approximately 2.5 liters of water).'},
      {'tag': 'Monitoring', 'text': 'SafeLife will alert you if any indirect kidney risk indicators change.'},
      {'tag': 'Consultation', 'text': 'Consider annual kidney function blood tests, especially if you have diabetes or hypertension risk factors.'},
    ];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
...
class _DataTransparency extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final sources = ['Blood pressure estimation', 'Heart rate patterns', 'Activity tracker', 'Hydration inference'];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
        padding: const EdgeInsets.all(16),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const Text('30 days', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              ...
              children: [
                ...
                const Text('72%', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 6),
            ClipRRect(
              borderRadius: BorderRadius.circular(3),
              child: const LinearProgressIndicator(value: 0.72, minHeight: 4, backgroundColor: _C.grey3, valueColor: AlwaysStoppedAnimation(_C.green)),
            ),
            const SizedBox(height: 14),
            Container(
              padding: const EdgeInsets.all(12),
              ...
              child: const Row(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Icon(Icons.warning_amber_rounded, color: _C.warnIcon, size: 15),
                  SizedBox(width: 8),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Missing Data', style: TextStyle(color: _C.warnIcon, fontSize: 12, fontWeight: FontWeight.w700)),
                        SizedBox(height: 3),
                        Text('Kidney function requires clinical blood/urine tests for definitive assessment', style: TextStyle(color: _C.warnIcon, fontSize: 11.5, height: 1.4)),
                      ],
                    ),
                  ),
                ],
            ...
            child: InkWell(
              onTap: () {
                if (item['label'] == 'Pattern Analysis') {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const PatternAnalysisScreen(riskType: 'Kidney Risk')),
                  );
                } else if (item['label'] == 'AI Explanation') {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const AiExplanationScreen(riskType: 'Kidney Risk')),
                  );
                } else if (item['label'] == 'Prevention Guidance') {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const PreventionGuidanceScreen(riskType: 'Kidney Risk')),
                  );
                } else if (item['label'] == 'Medical Preparation') {
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const MedicalPreparationScreen(riskType: 'Kidney Risk')),
                  );
                }
              },
