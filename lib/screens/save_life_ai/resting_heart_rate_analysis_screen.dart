...
class RestingHeartRateAnalysisScreen extends StatefulWidget {
  const RestingHeartRateAnalysisScreen({super.key});

  @override
  State<RestingHeartRateAnalysisScreen> createState() =>
      _RestingHeartRateAnalysisScreenState();
}

class _RestingHeartRateAnalysisScreenState
    extends State<RestingHeartRateAnalysisScreen> {
          ...
          children: [
            ...
            Row(
              children: const [
                ...
                SizedBox(width: 6),
                Text(
                  'Within personal baseline range',
                  style: TextStyle(
                    color: _C.green,
                    fontSize: 12,
                    fontWeight: FontWeight.w500,
...
class _TrendChart extends StatelessWidget {
  static const _xLabels = ['02/10', '02/12', '02/14', '02/16', '02/18', '02/20', '02/23'];
  static const _actual = [68.0, 70.0, 67.0, 69.0, 71.0, 68.0, 68.0];
  static const _pred = [68.2, 69.5, 67.5, 68.8, 70.2, 68.3, 68.1];
  static const _base = [68.0, 68.0, 68.0, 68.0, 68.0, 68.0, 68.0];
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _section('What was detected', 'Your resting heart rate remains consistent with your established baseline. No arrhythmia patterns have been detected in the continuous monitoring data.'),
            _section('Why it matters', 'Resting heart rate is a fundamental indicator of cardiac health. Sudden changes or irregular patterns can signal arrhythmias or other cardiac conditions early.'),
            _section('Short-term meaning', 'Your cardiac rhythm is regular and your resting rate is within a healthy range for your profile.'),
            _section('Long-term prediction', 'Your consistent resting heart rate trend suggests stable cardiac function. Continue current lifestyle habits.'),
            const SizedBox(height: 16),
            GestureDetector(
              onTap: onToggle,
              child: Container(
...
class _ContributingFactors extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final factors = [
      {'label': 'Fitness Level', 'low': true},
      {'label': 'Sleep Quality', 'low': true},
      {'label': 'Stress Levels', 'low': true},
    ];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
...
class _RecommendedActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final actions = [
      {'tag': 'Lifestyle', 'text': 'Continue your current exercise routine to maintain cardiac fitness.'},
      {'tag': 'Monitoring', 'text': 'SafeLife will alert you if any rhythm irregularities are detected.'},
    ];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
...
class _DataTransparency extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final sources = ['Optical heart rate sensor (PPG)', 'Accelerometer', 'Gyroscope'];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
        padding: const EdgeInsets.all(16),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const Text('30 days continuous', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              ...
              children: [
                ...
                const Text('84%', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 6),
            ClipRRect(
              borderRadius: BorderRadius.circular(3),
              child: const LinearProgressIndicator(value: 0.84, minHeight: 4, backgroundColor: _C.grey3, valueColor: AlwaysStoppedAnimation(_C.green)),
            ),
          ],
        ),
      ),
            ...
            child: InkWell(
              onTap: () {
                if (item['label'] == 'Pattern Analysis') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const PatternAnalysisScreen(riskType: 'Resting Heart Rate')));
                } else if (item['label'] == 'AI Explanation') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const AiExplanationScreen(riskType: 'Resting Heart Rate')));
                } else if (item['label'] == 'Prevention Guidance') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const PreventionGuidanceScreen(riskType: 'Resting Heart Rate')));
                } else if (item['label'] == 'Medical Preparation') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const MedicalPreparationScreen(riskType: 'Resting Heart Rate')));
                }
              },
