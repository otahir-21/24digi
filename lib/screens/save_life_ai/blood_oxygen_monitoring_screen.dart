...
class BloodOxygenMonitoringScreen extends StatefulWidget {
  const BloodOxygenMonitoringScreen({super.key});

  @override
  State<BloodOxygenMonitoringScreen> createState() =>
      _BloodOxygenMonitoringScreenState();
}

class _BloodOxygenMonitoringScreenState
    extends State<BloodOxygenMonitoringScreen> {
          ...
          children: [
            ...
            Row(
              children: const [
                ...
                SizedBox(width: 6),
                Text(
                  'Consistently normal oxygen levels',
                  style: TextStyle(
                    color: _C.green,
                    fontSize: 12,
                    fontWeight: FontWeight.w500,
...
class _TrendChart extends StatelessWidget {
  ...
  static const _actual = [97.0, 96.5, 98.0, 97.5, 97.0, 97.5, 97.0];
  static const _pred = [97.2, 96.8, 97.5, 97.2, 97.1, 97.3, 97.2];
  static const _base = [97.0, 97.0, 97.0, 97.0, 97.0, 97.0, 97.0];
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _section('What was detected', 'Your blood oxygen saturation levels remain consistently within the normal range (95-100%). No desaturation events have been detected during daytime or nighttime monitoring.'),
            _section('Why it matters', 'Blood oxygen saturation reflects how well your lungs are transferring oxygen to your blood. Persistent low levels can indicate respiratory or circulatory issues.'),
            _section('Short-term meaning', 'Your oxygen levels are healthy. Your body is efficiently oxygenating your blood.'),
            _section('Long-term prediction', 'No respiratory concerns indicated by current trends. Monitoring will continue automatically.'),
            const SizedBox(height: 16),
            GestureDetector(
              onTap: onToggle,
              child: Container(
...
class _ContributingFactors extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final factors = [
      {'label': 'Respiratory Rate', 'low': true},
      {'label': 'Activity Level', 'low': true},
    ];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
...
class _RecommendedActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final actions = [
      {'tag': 'Lifestyle', 'text': 'Continue regular physical activity to support lung health.'},
      {'tag': 'Monitoring', 'text': 'SafeLife continuously tracks your SpO2. No additional monitoring needed.'},
    ];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
...
class _DataTransparency extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final sources = ['Pulse oximeter (SpO2 sensor)', 'Heart rate sensor', 'Respiratory rate estimation'];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
        padding: const EdgeInsets.all(16),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const Text('89%', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 6),
            ClipRRect(
              borderRadius: BorderRadius.circular(3),
              child: const LinearProgressIndicator(value: 0.89, minHeight: 4, backgroundColor: _C.grey3, valueColor: AlwaysStoppedAnimation(_C.green)),
            ),
          ],
        ),
      ),
            ...
            child: InkWell(
              onTap: () {
                if (item['label'] == 'Pattern Analysis') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const PatternAnalysisScreen(riskType: 'Blood Oxygen Monitoring')));
                } else if (item['label'] == 'AI Explanation') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const AiExplanationScreen(riskType: 'Blood Oxygen Monitoring')));
                } else if (item['label'] == 'Prevention Guidance') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const PreventionGuidanceScreen(riskType: 'Blood Oxygen Monitoring')));
                } else if (item['label'] == 'Medical Preparation') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const MedicalPreparationScreen(riskType: 'Blood Oxygen Monitoring')));
                }
              },
