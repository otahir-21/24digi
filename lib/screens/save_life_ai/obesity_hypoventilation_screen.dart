...
class ObesityHypoventilationScreen extends StatefulWidget {
  const ObesityHypoventilationScreen({super.key});

  @override
  State<ObesityHypoventilationScreen> createState() =>
      _ObesityHypoventilationScreenState();
}

class _ObesityHypoventilationScreenState
    extends State<ObesityHypoventilationScreen> {
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            _section('What was detected', 'SafeLife has not detected significant nighttime hypoventilation patterns. Your nocturnal oxygen levels and respiratory rate remain within normal parameters.'),
            _section('Why it matters', 'Obesity hypoventilation syndrome (OHS) involves inadequate breathing during sleep, leading to dangerously high CO2 levels. Early detection prevents serious complications.'),
            _section('Short-term meaning', 'Your nighttime breathing and oxygenation are functioning well. No signs of hypoventilation detected.'),
            _section('Long-term prediction', 'Current trends suggest low risk. SafeLife will continue monitoring for any changes.'),
            const SizedBox(height: 16),
            GestureDetector(
              onTap: onToggle,
              child: Container(
...
class _RecommendedActions extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final actions = [
      {'tag': 'Lifestyle', 'text': 'Maintain current weight management habits.'},
...
class _DataTransparency extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    final sources = ['SpO2 sensor', 'Respiratory rate estimation', 'Activity tracker', 'Sleep monitor'];
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16),
      child: Container(
        padding: const EdgeInsets.all(16),
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                const Text('30 nights', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 12),
            Row(
              ...
              children: [
                ...
                const Text('78%', style: TextStyle(color: _C.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ],
            ),
            const SizedBox(height: 6),
            ClipRRect(
              borderRadius: BorderRadius.circular(3),
              child: const LinearProgressIndicator(value: 0.78, minHeight: 4, backgroundColor: _C.grey3, valueColor: AlwaysStoppedAnimation(_C.green)),
            ),
          ],
        ),
      ),
            ...
            child: InkWell(
              onTap: () {
                if (item['label'] == 'Pattern Analysis') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const PatternAnalysisScreen(riskType: 'Obesity Hypoventilation')));
                } else if (item['label'] == 'AI Explanation') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const AiExplanationScreen(riskType: 'Obesity Hypoventilation')));
                } else if (item['label'] == 'Prevention Guidance') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const PreventionGuidanceScreen(riskType: 'Obesity Hypoventilation')));
                } else if (item['label'] == 'Medical Preparation') {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const MedicalPreparationScreen(riskType: 'Obesity Hypoventilation')));
                }
              },
