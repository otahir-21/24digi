...
| **Parser merge** | `lib/bracelet/data/bracelet_data_parser.dart` | **`_mergeIntoNightlySessions`** (e.g. 347+), **`_selectBestMergedSession`** (455+): merge by same-day, gap ≤ 45 min; choose valid session with largest totalSleepMinutes. **Merge is correct; no device in key.** |
