...
public class PieChartRenderer extends AbstractChartRenderer {
	...
	private TextPaint centerCircleText3Paint = new TextPaint();
	private FontMetricsInt centerCircleText3FontMetrics = new FontMetricsInt();
	...
	public PieChartRenderer(Context context, Chart chart, PieChartDataProvider dataProvider) {
		...
		centerCircleText3Paint.setAntiAlias(true);
		centerCircleText3Paint.setTextAlign(Align.CENTER);
	}
	...
	public void initDataAttributes() {
		...
		if (null != data.getCenterText3Typeface()) {
			centerCircleText3Paint.setTypeface(data.getCenterText3Typeface());
		}
		centerCircleText3Paint.setTextSize(Utils.sp2px(scaledDensity, data.getCenterText3FontSize()));
		centerCircleText3Paint.setColor(data.getCenterText3Color());
		centerCircleText3Paint.getFontMetricsInt(centerCircleText3FontMetrics);
	}
	...
	private static final String TAG = "PieChartRenderer";
	/**
	...
	private void drawCenterCircle(Canvas canvas) {
		...
		List<ArcValue>list= data.getValues();

		 float centerX = drawCircleOval.centerX();
		 float centerY = drawCircleOval.centerY();

//		if(list.size()!=0){
//			ArcValue arcValue=list.get(0);
//			final int arcSpacing = Utils.dp2px(density, arcValue.getArcSpacing());
//
//			centerX=centerX+(arcVector.x * arcSpacing)/2f;
//			centerY=centerY+(arcVector.y * arcSpacing)/2f;
//		}


		canvas.drawCircle(centerX, centerY, centerRadius, centerCirclePaint);
		//if(data.getCenterCircleWidth()!=0)
		//canvas.drawCircle(centerX,centerY,centerRadius,centerCirclePaint2);
		...
		if (!TextUtils.isEmpty(data.getCenterText1())) {
			...
			String CenterText2=data.getCenterText2();
			if (!TextUtils.isEmpty(CenterText2)) {
				// Draw text 2 only if text 1 is not empty.
				String centerText3=data.getCenterText3();
				int padding=Utils.dp2px(density,16);
				...
				String value=CenterText2;
				textPaint.getTextBounds(value, 0, value.length(), rect);
				int height = rect.height();
				canvas.translate(centerX,centerY-height);
				StaticLayout staticLayout1 = new StaticLayout(value
						, textPaint, 600,
						Layout.Alignment.ALIGN_NORMAL, 1, 0, true);
				staticLayout1.draw(canvas);
				canvas.restore();
				canvas.drawText(data.getCenterText1(), centerX, centerY - height/2-padding, centerCircleText1Paint);

				if (!TextUtils.isEmpty(centerText3)) {
					Rect rect3 = new Rect();
					centerCircleText3Paint.setColor(data.getCenterText3Color());
					centerCircleText3Paint.getTextBounds(centerText3, 0, centerText3.length(), rect3);
					canvas.drawText(centerText3, centerX, centerY+height/2+padding+rect3.height()/2, centerCircleText3Paint);
				}
			} else {
	...
	private void drawArcs(Canvas canvas, int mode) {
		...
		int arcIndex = 0;
		int spaceAngel=data.getValues().size()==1?0:1;
		for (ArcValue arcValue : data.getValues()) {
			final float angle = Math.abs(arcValue.getValue()) * arcScale-spaceAngel;
			if (MODE_DRAW == mode) {
